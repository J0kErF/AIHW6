...
- [x] F1. Choose platform + create account: try **Prefect Cloud** (spec's example) first;
...
## Tasks (post SP-3)
- [x] F4. Deploy cop + thief servers → record `cop_mcp_url`, `thief_mcp_url` in config.
- [x] F5. Security proofs: curl without token → 401 (save output); with token → 200;
...
- [x] F7. Cloud E2E: orchestrator local (hybrid Approach 3, outbound-only) → full 6-sub-game
